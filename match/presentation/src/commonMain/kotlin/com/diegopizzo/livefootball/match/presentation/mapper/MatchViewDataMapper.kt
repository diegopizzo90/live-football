...
import com.diegopizzo.livefootball.match.domain.repository.model.MatchStatus.EXTRA_TIME
import com.diegopizzo.livefootball.match.domain.repository.model.MatchStatus.FIRST_HALF_KICK_OFF
import com.diegopizzo.livefootball.match.domain.repository.model.MatchStatus.NOT_AVAILABLE
import com.diegopizzo.livefootball.match.domain.repository.model.MatchStatus.NOT_STARTED
import com.diegopizzo.livefootball.match.domain.repository.model.MatchStatus.SECOND_HALF_STARTED
            ...
            return LFCardMatchViewData(
                match = LFCellMatchViewData(
                    ...
                    cellIconHome = LFCellIconViewData(
                        icon = LFIconViewData(
                            painter = PainterResource.Url(teams.home.logo),
                        ),
                        text = teams.home.name,
                    ),
                    cellIconAway = LFCellIconViewData(
                        icon = LFIconViewData(
                            painter = PainterResource.Url(teams.away.logo),
                        ),
    ...
    private fun mapChipsViewData(data: LeagueData, isSelected: Boolean): LFChipViewData {
        with(data) {
            return LFChipViewData(
                id = id,
                icon = logo?.let { LFIconViewData(painter = PainterResource.Url(it)) },
                text = name,
                selected = isSelected,
            )
        }
